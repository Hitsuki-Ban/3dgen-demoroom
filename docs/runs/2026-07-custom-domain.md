...
2026-07-12 03:53 JST に Cloudflare Free Zone を作成した。Cloudflare quick scan は 0 records で、別途行った公開 DNS の照合でも apex の A / AAAA / CNAME / MX / TXT / CAA は空、`3dgen.hitsuki.space` は NXDOMAIN だった。公開 DS も空で、RDAP の `delegationSigned` は false だった。
...
1. `infra/cloudflare/hitsuki-space/` の preflight で既存 phase entrypoint がないことを確認し、Terraform で proxied apex DNS、Hunyuan geo WAF、apex redirect を適用する。
...
- mock provider の `terraform test -verbose` は 1/1 pass。plan は apex DNS、WAF ruleset、redirect ruleset の 3 add で、WAF の 29 country codes、host / path、redirect の 301 / path / query 契約を検査した。
- Worker と Terraform の country set は双方 29 unique codes で、差分 0 を機械比較した。
...
Phase 2 の token には少なくとも Zone Read、DNS Write、Zone WAF Write、Dynamic URL Redirects Write、Workers Scripts Write、Workers Routes Write が必要で、resource scope に新しい `hitsuki.space` zone を含める。Terraform state 用 R2 credentials は Cloudflare API token と分離し、環境変数だけで渡す。
