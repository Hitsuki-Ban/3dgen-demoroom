...
- 確定日: 2026-07-08(同日改訂: オーナー決定により Hunyuan3D 2.1 を geo 制限付き採用に変更。2026-07-12: Cloudflare 公式 EU country list に合わせ geo code を訂正)
...
- つまり**ブロックすべきは EU territory + UK + 韓国**。Cloudflare の geo 判定では EU 加盟国 27 コードだけでなく、別 ISO code の EU 領域 `AX/GF/GP/MF/MQ/RE/YT` も EU と判定されるため、Free plan では公式 34-code EU set + `GB/KR` の計 36 codes を列挙する。日本限定にする必要はなく、それ以外の全地域には公開できる
- MAU 条項(月間 100 万超で要追加許諾)は本サイト規模では非該当。AI 生成物である旨の表示は AUP 要件として全モデル共通で実施

### 実装方式(Cloudflare、2026-07-12 再確認済み)

- Hunyuan3D 2.1 の成果物(GLB / サムネイル / レンダリング画像)は**専用パスプレフィックス**(例 `/restricted/hunyuan3d-21/`)に隔離する
- **WAF カスタムルール**(Free プランで 5 本まで)1 本でブロック: `http.request.uri.path` が当該プレフィックス かつ `ip.src.country in {…Cloudflare 公式 EU 34 codes + GB + KR の36コード列挙…}`(`ip.src.is_in_european_union` フィールドは Business 以上のため、Free では[同フィールドの公式 country list](https://developers.cloudflare.com/ruleset-engine/rules-language/fields/reference/ip.src.is_in_european_union/)を列挙で再現する)
