# ベンチマーク課題セット設計 v1(ドラフト)

- 作成日: 2026-07-07
- 作成者: Fable
- ステータス: **ドラフト** — モデルラインナップ確定(#1 の突き合わせ)後に確定版にする
...
1. **基準環境**: 単一 GPU(RTX 5090 / 32GB を想定、#2 の突き合わせで確定)。互換性・OOM で不可能なモデルのみ代替 GPU を使い、その旨をメタデータと画面に明記
2. **設定は各リポジトリの公式推奨デフォルト**を使う。個別チューニングはしない。使った全パラメータを記録
3. **シード固定で 1 課題 × N 回生成**(N は §6 の未決事項)。失敗(クラッシュ・空出力)はリトライとしてカウントし、回数を記録
...
- [ ] **シード数 N**: 1(最安・分散を隠すリスク)vs 3〜4(生成のばらつきも展示できるが費用 ×N)。#2 の費用調査確定後に決める。暫定推奨: N=3
- [ ] キャラクター課題に T-pose 指定を入れるか(リギング前提の評価になるが、モデルによっては poses が乱れる)
- [ ] サイトの言語(日 / 英 / 両方)。読者層の想定次第 — オーナー判断を仰ぐ
- [ ] text-to-3D モデルと image-to-3D モデルの結果を同一行に並べる際の注意書きの文言
