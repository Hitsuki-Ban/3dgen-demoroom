...
def test_triposg_incremental_upload_uses_task_id_prefix(monkeypatch, tmp_path: Path) -> None:
    runner = _load_runner(monkeypatch)
    ...
    def fake_upload(self, source_dir: Path, relative_name: str = "") -> list[str]:
        ...
        return ["runs/triposg/wave1/test/task-a/meta.json"]

    monkeypatch.setenv("RUNPOD_INCREMENTAL_S3_TARGET", "s3://3dgen-runs/runs/triposg/wave1/test")
    ...
    assert runner.upload_task_increment_if_configured(task_dir, "task-a", dict(os.environ)) == [
        "runs/triposg/wave1/test/task-a/meta.json",
    ]
...
def test_run_task_upload_failure_does_not_retry_successful_triposg_task(monkeypatch, tmp_path: Path) -> None:
    runner = _load_runner(monkeypatch)
    input_root = tmp_path / "input"
    output_root = tmp_path / "output"
    input_root.mkdir()
...
def test_run_task_timeout_writes_and_uploads_failure_before_reraising(monkeypatch, tmp_path: Path) -> None:
    ...
    def fake_run_with_peak_vram(
        ...
        *,
        ...
        raise runner_utils.RunnerTimeoutError(
            ...
            output_tail="CUDA kernel still running",
        )
    ...
    def fake_upload(task_output_dir: Path, task_id: str, env: dict[str, str]) -> list[str]:
        ...
        assert failure["error_output_tail"] == "CUDA kernel still running"
        assert (task_output_dir / "infer.log").read_text(encoding="utf-8") == "CUDA kernel still running\n"
