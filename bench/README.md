...
- The runner publishes each task through the staged task protocol. The runtime writes `runpod-startup.json`, `runpod-status.json`, and `runpod-runner.log` only under `/work/runpod-telemetry`, waits for the ownership handoff, and runs the model wrapper with task output under `/work/output`. It uploads a `finalizing` status (with provisional `outcome`) in a telemetry-only sweep, then replaces only `runpod-status.json` with terminal `ok`/`failed`. If the telemetry sweep or status-only publish fails, the remote status remains `finalizing`. Cleanup and DELETE are attempted only after those evidence operations. Upload/status failures take precedence over the runner exit code; termination failures never hide the earlier failure.
